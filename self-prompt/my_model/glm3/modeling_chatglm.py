...
class Embedding(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        if self.is_train_sys_prompt_mode:
            if self.w_emb is None:
                self.w_emb = nn.Embedding(
                    ...
                    device=self.device,
                    _weight=copy.deepcopy(self.word_embeddings.weight.detach())
                )
    ...
    def my_train_sys_emb(self, input_ids):
        ...
        if not self.sys_prompt_is_list:
            ...
            sys_emb_front = self.word_embeddings(sys_tensor_front)
            sys_emb_back = self.word_embeddings(sys_tensor_back)
