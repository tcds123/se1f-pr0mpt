...
def train(args):
    ...
    name_and_params = list(clip_l.named_parameters())
    # for n, param in name_and_params:
    #     if 'token_embedding' in n:
    #         name_and_params.append((n, param))
    ...
    if args.deepspeed:
        ...
        clip_l = accelerator.prepare(clip_l, device_placement=[not is_swapping_blocks])
        if is_swapping_blocks:
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                self_prompt.update_dict(train_dataloader[step], loss)
...
class SelfPrompt:
    def __init__(self, model):
        ...
        self.grad = None
        self.pre_loss = None
        self.relate_dict = defaultdict(Counter)
        self.sp_token = [267, 49406, 49407]  # [, , <|startoftext|>, <|endoftext|>]

    def get_relate_dict(self):
        return self.relate_dict
    ...
    def update_dict(self, input_ids, loss):
        ...
        if self.grad is None:
            ...
            if loss < self.pre_loss:
                for id in input_ids[0][0]:
                    id = id.item()
                    for res_id in input_ids[0][0]:
                        res_id = res_id.item()
                        if id == res_id:
                            continue
                        if id in self.sp_token or res_id in self.sp_token:
                            continue
                        self.relate_dict[id][res_id] += 1
            elif loss > self.pre_loss:
                for id in input_ids[0][0]:
                    id = id.item()
                    for res_id in input_ids[0][0]:
                        res_id = res_id.item()
