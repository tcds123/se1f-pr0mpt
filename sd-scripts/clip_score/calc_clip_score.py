...
class ClipScore:
    def __init__(self, clip_model, device=None, num_workers=None):
        if device is None:
    ...
    def call_main(self,
        ...
        dataset = DummyDataset(real_path, fake_path,
                               real_flag, fake_flag,
                               transform=self.preprocess, tokenizer=clip.tokenize)
        dataloader = DataLoader(dataset, batch_size,
                                num_workers=self.num_workers, pin_memory=True)

        print('Calculating CLIP Score:')
        clip_score = calculate_clip_score(dataloader, self.model,
                                          real_flag, fake_flag)
        clip_score = clip_score.cpu().item()
        print('CLIP Score: ', clip_score)
        return clip_score
